...
def generate_task_readme(task: TaskInfo, out_dir: Path) -> Path:
    ...
    content = dedent(f"""\
        ...
        > **Group {task.group} -- {task.group_label}**

        Task folder: `TASKS/{task.task_name}/`
        ...
        | **Group** | {task.group} |
        | **Group Label** | {task.group_label} |
        | **DB2** | {'Yes' if any(p.has_db2 for p in task.programs) else 'No'} |
        | **VSAM KSDS** | {'Yes' if (task.jcl_defksds or any(p.vsam_ksds for p in task.programs)) else 'No'} |
        | **VSAM AIX** | {'Yes' if (task.jcl_defaix or any(p.vsam_aix for p in task.programs)) else 'No'} |
        | **VSAM ESDS** | {'Yes' if (task.jcl_defesds or any(p.vsam_esds for p in task.programs)) else 'No'} |
        | **GDG** | {'Yes' if task.jcl_defgdg else 'No'} |
        | **QMF** | {'Yes' if task.jcl_dsqqmfe else 'No'} |
        | **Multi-program CALL** | {'Yes' if any(p.calls for p in task.programs) else 'No'} |
        | **Shared Copybook** | {', '.join(f'`{c}`' for c in task.copybooks) if task.copybooks else 'No'} |

        ---

        _Auto-generated by `cobol-knowledge-builder`. Source of truth: `TASKS/{task.task_name}/`._
    """)
